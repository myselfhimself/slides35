...
from slides35 import Slide, SLIDES35_DEFAULT_SVG_TEMPLATE, SLIDES35_CLI_DEFAULT_CONVERT_PNG_DENSITY_DPI
...
def test_command_stdout_svg_output(capsys):
    ...
    command_output = subprocess.check_output(['python', EXECUTABLE_UNDER_TEST, '--id', '1', '--picture', DEFAULT_PICTURE])
    assert _normalize_xml(command_output) == _normalize_xml(Slide(DEFAULT_SLIDE_TEMPLATE).id(1).picture(DEFAULT_PICTURE).svg())

def test_command_file_svg_output():
    output_svg_path = 'test1.svg'
    subprocess.run(['python', EXECUTABLE_UNDER_TEST, '--id', '1', '--picture', DEFAULT_PICTURE, '--output', output_svg_path])
    with open(output_svg_path) as f:
        output_svg_string = f.read()
    assert _normalize_xml(output_svg_string) == _normalize_xml(Slide(DEFAULT_SLIDE_TEMPLATE).id(1).picture(DEFAULT_PICTURE).svg())
...
def test_command_file_png_output():
    ...
    dpi = 400
    assert dpi != SLIDES35_CLI_DEFAULT_CONVERT_PNG_DENSITY_DPI # avoid false positive because of same value as default
    subprocess.run(['python', EXECUTABLE_UNDER_TEST, '--id', '1', '--picture', DEFAULT_PICTURE, '--output', output_png_path, '--dpi', str(dpi)])
